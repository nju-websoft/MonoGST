...
- `make MonoGST`  or `g++ -std=c++20 -O2 -g -Isrc/MonoGST/include -DDEBUG -o MonoGST src/MonoGST/tests/test_Improved2star.cpp src/MonoGST/src/GlobalUtils.cpp src/MonoGST/src/Log.cpp` to compile $\mathtt{MonoGST+}$.
- `make MonoGST_abl` or `g++ -std=c++20 -O2 -g -Isrc/MonoGST/include -DDEBUG -o MonoGST_abl src/MonoGST/tests/test_Abl.cpp src/MonoGST/src/GlobalUtils.cpp src/MonoGST/src/Log.cpp` to compile the ablation experiments of $\mathtt{MonoGST+}$.
...
Run $\mathtt{MonoGST+}$ for `example`: `./MonoGST example`.

Run the ablation experiments of $\mathtt{MonoGST+}$ for `example`: `./MonoGST example x`, `x` is a parameter to control the variants of $\mathtt{MonoGST+}$. If $x=0$, it will run $\mathtt{MonoGST}$; if $x=6$, it will run `w/o suspension`, ; if $x=5$, it will run `w/o mono.`; if $x=3$, it will run `trivial edges`; if $x=7$, it will run $\mathtt{MonoGST+}$.
